			...
			<div class="row justify-content-center">
				<div class="col-md-8">
					<div class="card">
						...
						<div class="card-body">
							<form id="updateForm" action="<c:url value='/board/update' />"
								method="post">
								<input id="boardNoInput" type="hidden" name="boardNo"
									value="${board.boardNo}">
								<!-- 제목 -->
								<div class="mb-3">
								...
								<div class="mb-3">
									...
									<textarea id="contentInput" class="form-control" name="content"
								...
								<!-- 작성자 ID -->
								<div class="mb-3">
									<label class="form-label"><strong>작성자 ID</strong></label> <input
										type="text" class="form-control" value="${board.memberId}"
										readonly>
								</div>

								<!-- 영화 선택 -->
								<div class="mb-3">
									<label for="movieSelect" class="form-label">영화 선택</label> <select
										id="movieSelect" name="movieId" required>
										<option value="" disabled selected>영화를 선택하세요</option>
										<c:forEach var="movie" items="${movieList}">
											<option value="${movie.movieId}"
												${movie.movieId == board.movieId ? 'selected' : ''}>${movie.name}</option>
										</c:forEach>
									</select>
								</div>
								...
								<div class="mb-3">
									...
									<input type="checkbox" id="spoilerCheck" name="spoiler"
										value="Y">
									<div class="form-text" style="font-size: 12px;">스포일러가 포함된
										경우 체크해주세요.</div>
								</div>
	...
	document
			...
			.addEventListener(
					...
					function(event) {
						...
						// 스포일러 체크박스 상태 확인
						const spoilerChecked = document
								.getElementById("spoilerCheck").checked;

						// 스포일러가 체크되지 않았다면 N으로 설정
						if (!spoilerChecked) {
							const spoilerInput = document
									.createElement("input");
